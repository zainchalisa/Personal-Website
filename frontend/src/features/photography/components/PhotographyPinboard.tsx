...
import type { Theme, ThemeTransition } from '@/hooks/themeTransition'
...
import { preloadImage, clearImagePreloadCache } from '@/lib/preloadImage'
import { PinboardCardImage } from './PinboardCardImage'
import {
  preloadCountryGallery,
  preloadRegionCardThumbs,
  preloadRegionGallery,
} from './photographyPreload'
import { useSlideshowImages } from './useSlideshowImages'
...
export default function PhotographyPinboard({
  ...
  const beginPinch = useCallback((): boolean => {
    ...
    if (startDist < MIN_PINCH_START_DIST) return false
  ...
  const startSinglePointerDrag = useCallback((clientX: number, clientY: number) => {
    interactingRef.current = true
    flyRef.current = null
    rubberBandSnapRef.current = null
    inertiaRef.current.active = false
    velocitySampleBufferRef.current = []
    lastPointerRef.current = { x: clientX, y: clientY, t: performance.now() }
    dragRef.current = {
      active: true,
      didDrag: false,
      sx: clientX,
  ...
  const handleFramePointerDown = useCallback(
    (e: ReactPointerEvent) => {
      ...
      if (pointersRef.current.size === 1) {
        startSinglePointerDrag(e.clientX, e.clientY)
      } else if (pointersRef.current.size === 2) {
        if (!beginPinch()) {
          const staleId = [...pointersRef.current.keys()][0]
          pointersRef.current.delete(staleId)
    ...
    [beginPinch, startSinglePointerDrag],
  )
  ...
  const onCardPointerDown = useCallback(
    ...
    [isMobile, handleFramePointerDown],
  )

  const onCardPointerUp = useCallback(
    (e: ReactPointerEvent) => {
      if (isMobile) handleFramePointerUp(e)
      e.stopPropagation()
    },
    [isMobile, handleFramePointerUp],
  )
  ...
  useSlideshowImages(slideshow?.photos, slideIndex, { active: Boolean(slideshow) })
            ...
            return (
              ...
              >
                ...
                <button
                  ...
                  onPointerEnter={() => {
                    const firstSrc =
                      c.card.previewPhoto?.thumbSrc ??
                      c.card.previewPhoto?.src ??
                      c.card.photos.find((photo) => photo.src)?.src
                ...
                >
                  ...
                  />
                  {c.card.previewPhoto?.thumbSrc || c.card.previewPhoto?.src ? (
                    <PinboardCardImage
                      thumbSrc={
                        c.card.previewPhoto.thumbSrc ?? c.card.previewPhoto.src!
                      }
                      displaySrc={c.card.previewPhoto.src}
                      originalSrc={c.card.previewPhoto.originalSrc}
                      width={c.w}
                      height={imgH}
                      priority={activeRegionId === r.id}
                    />
                  ) : (
                    <svg
            ...
            >
              {currentPhoto?.src ? (
                <SlideshowPhoto
                  ...
                  fallbackSrc={currentPhoto.originalSrc}
                  alt={slideCaption}
