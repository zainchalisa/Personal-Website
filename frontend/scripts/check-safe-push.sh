...
is_blocked_path() {
  case "$1" in
    .env.example|frontend/.env.example) return 1 ;;
    .env|frontend/.env) return 0 ;;
    ASSET_HOSTING.md|docs/architecture/ASSET_HOSTING.md) return 0 ;;
    .dev.vars|frontend/.dev.vars) return 0 ;;
    dist/*|frontend/dist/*) return 0 ;;
    node_modules/*|frontend/node_modules/*) return 0 ;;
    .wrangler/*|frontend/.wrangler/*) return 0 ;;
    .vercel/*|frontend/.vercel/*) return 0 ;;
  esac
  case "$1" in
    .env.*) return 0 ;;
    frontend/.env.*) return 0 ;;
  esac
  return 1
}
...
if [[ -f frontend/.env ]] && ! git check-ignore -q frontend/.env 2>/dev/null; then
  fail "frontend/.env exists but is not listed in .gitignore"
fi

if git ls-files --error-unmatch frontend/.env >/dev/null 2>&1; then
  fail "frontend/.env is tracked by git — run: git rm --cached frontend/.env"
fi
...
if git ls-files --error-unmatch docs/architecture/ASSET_HOSTING.md >/dev/null 2>&1; then
  fail "docs/architecture/ASSET_HOSTING.md is tracked — run: git rm --cached docs/architecture/ASSET_HOSTING.md"
fi
